...
class TestKuzu:
    ...
    def test_kuzu(self, mock_llm_factory, mock_embedder_factory, mock_config, mock_embedding_model, mock_llm):
        ...
        assert {f"{result['source']}_{result['relationship']}_{result['target']}" for result in results} == {
            "alice_knows_bob",
            "bob_knows_charlie",
            "charlie_likes_alice",
            "charlie_knows_alice",
        ...
        assert {f"{result['source']}_{result['relationship']}_{result['destination']}" for result in results} == {
            "alice_knows_bob",
            "bob_knows_charlie",
        }
