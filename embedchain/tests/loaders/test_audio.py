...
def setup_audio_loader(mocker):
    mock_deepgram = mocker.patch("deepgram.DeepgramClient")
...
def test_load_data_from_url(setup_audio_loader):
    loader, mock_client = setup_audio_loader
    ...
    mock_client.listen.rest.v.return_value.transcribe_url.assert_called_once_with(
        {"url": url}, model="nova-2", smart_format=True
    )
...
def test_load_data_from_file(setup_audio_loader):
    ...
    assert result == expected_result
    mock_client.listen.rest.v.assert_called_once_with("1")
    mock_client.listen.rest.v.return_value.transcribe_file.assert_called_once_with(
        {"buffer": mock_file.return_value}, model="nova-2", smart_format=True
    )
