...
class LLMConfig(BaseModel):
    ...
    api_key: str | None = Field(None, description="API key or 'env:API_KEY' to use environment variable")
...
async def reset_configuration(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to reset configuration: {e!s}")
