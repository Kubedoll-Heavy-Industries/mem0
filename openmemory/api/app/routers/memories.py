...
async def list_memories(
    ...
    sort_column: str | None = Query(None, description="Column to sort by (memory, categories, app_name, created_at)"),
    sort_direction: str | None = Query(None, description="Sort direction (asc or desc)"),
...
async def delete_memories(request: DeleteMemoriesRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"Memory service unavailable: {client_error!s}")
